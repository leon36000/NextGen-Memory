...
The gate composes with the r3 Review Attestation Registry public API. It does not duplicate reviewer registration or aggregate reviewer verdicts. It independently verifies that the supplied registry request, summary, and decision agree before using the review state.
...
The immutable RED branch is based on the design/plan commit above r3 and adds only:
...
- exact r3 base, ancestry, and nine-path surface;
- Ruff and compileall;
