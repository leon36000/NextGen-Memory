...
class PolicyPromotionReason(StrEnum):
    ...
    NON_POSITIVE_CONFIDENCE_LOWER_BOUND = (
        "non_positive_confidence_lower_bound"
    )
...
class PairedPolicyEvidence:
    ...
    def __post_init__(self) -> None:
        ...
        matched_pair_count = _nonnegative_integer(
        ...
        standard_error = _nonnegative_number(
            "score_standard_error", self.score_standard_error
        )
        mean_token_delta = _finite_number(
            "mean_token_delta", self.mean_token_delta
        )
        mean_latency_delta_ms = _finite_number(
        ...
        registry_pair_count = _nonnegative_integer(
        ...
        object.__setattr__(
            self, "treatment_policy_version", treatment_version
        )
        object.__setattr__(self, "matched_pair_count", matched_pair_count)
        object.__setattr__(self, "mean_score_effect", mean_score_effect)
        object.__setattr__(
            self, "score_confidence_lower_bound", lower
        )
        object.__setattr__(
            self, "score_confidence_upper_bound", upper
        )
        object.__setattr__(
            self, "score_standard_error", standard_error
        )
        object.__setattr__(self, "mean_token_delta", mean_token_delta)
        object.__setattr__(
            self, "mean_latency_delta_ms", mean_latency_delta_ms
        )
        ...
        object.__setattr__(
            self, "registry_failed_count", registry_failed_count
        )
        object.__setattr__(
            self, "registry_cancelled_count", registry_cancelled_count
        )
        object.__setattr__(
            self, "registry_active_count", registry_active_count
        )
        object.__setattr__(self, "content_hash", _hash_payload(self.to_dict()))
...
class PolicyOperationalReadiness:
    ...
    def __post_init__(self) -> None:
        ...
        reviewer_count = _nonnegative_integer(
...
class PolicyPromotionGateConfig:
    ...
    def __post_init__(self) -> None:
        ...
        maximum_harm_rate = _bounded_rate(
        ...
        gate_policy_version = _identifier(
        ...
        object.__setattr__(
            self, "maximum_mean_token_delta", maximum_mean_token_delta
        )
        object.__setattr__(
            self,
            "maximum_mean_latency_delta_ms",
        ...
        object.__setattr__(
            self, "minimum_reviewer_count", minimum_reviewer_count
        )
        object.__setattr__(
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "candidate_policy_content_hash": (
                self.candidate_policy.content_hash
            ),
...
class PolicyPromotionRecord:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.decision, PolicyPromotionDecision):
            raise PolicyPromotionValidationError(
                "decision must be a PolicyPromotionDecision"
            )
        reasons = tuple(self.reasons)
        if not reasons or any(
            not isinstance(reason, PolicyPromotionReason)
        ...
        if self.advisory_only is not True:
            raise PolicyPromotionValidationError(
                "promotion records must remain advisory only"
            )
    ...
    def _identity_payload(self) -> dict[str, object]:
        return {
            ...
            "current_policy_content_hash": (
                self.current_policy_content_hash
            ),
            "candidate_policy_content_hash": (
                self.candidate_policy_content_hash
            ),
            "evaluation_evidence_content_hash": (
                self.evaluation_evidence_content_hash
            ),
...
class AdvisoryPolicyPromotionGate:
    ...
    def evaluate(
        ...
        if not isinstance(request, PolicyPromotionRequest):
            raise PolicyPromotionValidationError(
                "request must be a PolicyPromotionRequest"
            )
        ...
        if (
            ...
            or not _matches_treatment_identity(candidate, evidence)
        ):
            reject_reasons.append(
                PolicyPromotionReason.CANDIDATE_POLICY_IDENTITY_MISMATCH
            )
        if (
            evidence.registry_completed_trial_count
            != evidence.matched_pair_count
        ):
            reject_reasons.append(
        ...
        if (
            ...
            > config.maximum_mean_latency_delta_ms
        ):
            reject_reasons.append(
        ...
        if (
            evidence.score_confidence_lower_bound
            <= config.minimum_score_lower_bound
        ):
            hold_reasons.append(
                PolicyPromotionReason.NON_POSITIVE_CONFIDENCE_LOWER_BOUND
            )
        if (
            evidence.score_standard_error
            > config.maximum_score_standard_error
        ):
            hold_reasons.append(
                PolicyPromotionReason.STANDARD_ERROR_EXCEEDED
            )
        if (
            readiness.evidence_age_seconds
            > config.maximum_evidence_age_seconds
        ):
...
def _matches_treatment_identity(
    ...
    return (
        ...
        and evidence.treatment_policy_fingerprint
...
def _identifier(name: str, value: object) -> str:
    if not isinstance(value, str) or _IDENTIFIER_RE.fullmatch(value) is None:
        raise PolicyPromotionValidationError(
            f"{name} must be a bounded policy identifier"
        )
    return value
