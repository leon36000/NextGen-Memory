...
class MergeReadinessConfig:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            ...
            _positive_int("minimum full-suite test count", self.minimum_full_suite_test_count),
        )
        object.__setattr__(
            self,
            "minimum_migration_pass_count",
            _nonnegative_int("minimum migration pass count", self.minimum_migration_pass_count),
        )
        object.__setattr__(self, "gate_policy_version", _policy_version(self.gate_policy_version))
...
class MergeCandidateIdentity:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "merge_policy_version", _policy_version(self.merge_policy_version))
        object.__setattr__(self, "content_hash", _hash_payload(self._payload()))
...
class ExactReviewReadinessEvidence:
    ...
    def _payload(self) -> dict[str, object]:
        return {
            "authenticated_envelope_evidence_sha256": (self.authenticated_envelope_evidence_sha256),
...
class MergeVerificationEvidence:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "candidate_sha", _git_sha("candidate SHA", self.candidate_sha))
        object.__setattr__(self, "diff_sha256", _sha256("diff SHA-256", self.diff_sha256))
        for name in (
        ...
        object.__setattr__(
            ...
            _optional_sha256("verification artifact", self.verification_artifact_sha256),
        )
        object.__setattr__(
            self,
            "integration_checkpoint_sha256",
            _optional_sha256("integration checkpoint", self.integration_checkpoint_sha256),
        )
        object.__setattr__(
            self,
    ...
    def _payload(self) -> dict[str, object]:
        return {
            ...
            "cross_python_semantic_identity_passed": (self.cross_python_semantic_identity_passed),
...
def _append_once(values: list[MergeReadinessReason], reason: MergeReadinessReason) -> None:
...
class ExactShaMergeReadinessGate:
    ...
    def evaluate(
        ...
        if (
            ...
            or (request_integrity and review_request.base_sha != candidate.expected_base_sha)
        ):
        ...
        if (
            ...
            or (request_integrity and review_request.diff_sha256 != candidate.expected_diff_sha256)
        ):
        ...
        if request_integrity and summary_integrity:
            expected_missing = max(0, review_request.minimum_approvals - summary.approval_count)
            if (
