...
def _freeze_metadata(metadata: Mapping[str, Any] | None) -> Mapping[str, Any]:
    frozen = _freeze_json(metadata or {})
...
def _freeze_json(value: Any) -> Any:
    if isinstance(value, Mapping):
        ...
        for key, nested in value.items():
            if not isinstance(key, str):
                raise ExecutionLedgerValidationError("metadata keys must be strings")
            normalized = key.lower()
            if normalized in _FORBIDDEN_METADATA_KEYS:
                raise ExecutionLedgerValidationError(
                    f"forbidden metadata key: {normalized}"
                )
    ...
    if value is None or isinstance(value, (bool, int, str)):
        return value
    raise ExecutionLedgerValidationError(
        f"metadata value type is not JSON-compatible: {type(value).__name__}"
    )
