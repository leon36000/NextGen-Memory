...
def evidence(index: int, **overrides: object) -> IntegratedContextEvidence:
    ...
    hash_character = "0123456789abcdef"[index % 16]
...
def test_empty_optional_pool_returns_mandatory_closure() -> None:
    ...
    candidates = (
        ...
        evidence(2, prerequisite_memory_ids=(memory_id(1),), mandatory=True),
    )

    problem, solution = solve(compile_request, candidates)
    ...
    assert set(solution.phase_by_id.values()) == {ContextSelectionPhase.MANDATORY}
...
def test_exact_solver_selects_positive_joint_synergy_with_negative_singletons() -> None:
    ...
    compile_request = request(objective_policy=policy)
    candidates = (
    ...
    problem, solution = solve(compile_request, candidates, interactions)
...
def test_redundancy_and_budget_select_only_one_representative() -> None:
    ...
    interactions = (
        pair(1, 2, kind=ContextInteractionKind.REDUNDANCY, value=-0.5),
    )
...
def test_harmful_high_relevance_evidence_is_excluded_and_budget_can_remain_unused() -> None:
    policy = ContextObjectivePolicy(
        new_expert_bonus=0.0,
        new_subject_bonus=0.0,
        new_source_cluster_bonus=0.0,
    ...
    assert solution.selected_ids == frozenset({memory_id(2)})
    assert evaluation.evidence_tokens < compile_request.usable_evidence_tokens
...
def test_prerequisite_chain_is_selected_atomically() -> None:
    ...
    candidates = (
        ...
        evidence(2, relevance=0.0, prerequisite_memory_ids=(memory_id(1),)),
        evidence(
            3,
            relevance=1.0,
...
def test_optional_expert_cap_is_respected() -> None:
    compile_request = request(max_items_per_expert=1)
    candidates = (
...
def test_ties_prefer_fewer_tokens_then_fewer_items_then_uuid() -> None:
    policy = ContextObjectivePolicy(
        new_expert_bonus=0.0,
        new_subject_bonus=0.0,
        new_source_cluster_bonus=0.0,
    )
    compile_request = request(max_items=2, objective_policy=policy)
    candidates = (
        ...
        evidence(4, relevance=0.25, estimated_tokens=25),
    )
...
def test_solution_marks_nonmandatory_items_exact_and_is_input_order_invariant() -> None:
    compile_request = request()
    candidates = (
    ...
    first_problem, first = solve(compile_request, candidates)
    second_problem, second = solve(compile_request, tuple(reversed(candidates)))
...
def test_exact_solver_matches_independent_oracle_on_500_generated_problems() -> None:
    rng = random.Random(20260814)
    solver = ExactContextSolver()

    for case_index in range(500):
        candidate_count = rng.randint(2, 7)
        ids = tuple(memory_id(case_index * 10 + index + 1) for index in range(candidate_count))
        generated: list[IntegratedContextEvidence] = []
        for index, item_id in enumerate(ids):
            prerequisites = ()
            if index and rng.random() < 0.25:
                prerequisites = (ids[rng.randrange(index)],)
            hash_character = "0123456789abcdef"[(case_index + index) % 16]
            generated.append(
                IntegratedContextEvidence(
                    memory_id=item_id,
                    space_id=SPACE_ID,
                    ...
                    content_hash=hash_character * 63 + f"{index % 16:x}",
            ...
            for right_index in range(left_index + 1, candidate_count):
                ...
                generated_interactions.append(
                    ContextPairInteraction(
                        ...
                        ids[right_index],
                        kind,
                        magnitude if kind is ContextInteractionKind.SYNERGY else -magnitude,
                        0.01,
                        3,
                        GROUP_ID,
                    )
        ...
        compile_request = IntegratedContextCompileRequest(
            ...
            envelope_tokens=50,
            max_items=rng.randint(2, candidate_count),
            coverage_demands=(ContextCoverageDemand("required", 2.0, True),),
            exact_candidate_limit=18,
        )
        problem = canonicalize_context_problem(
            compile_request,
        ...
        assert actual.selected_ids == expected.selected_ids


def test_exact_solver_rejects_nonproblem_and_candidate_count_above_limit() -> None:
    with pytest.raises(ContextCompilerValidationError, match="problem"):
        ExactContextSolver().solve(object())

    candidates = tuple(evidence(index) for index in range(1, 5))
    problem = canonicalize_context_problem(
