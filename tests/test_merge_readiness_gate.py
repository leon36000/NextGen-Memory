    ...
    *,
    ...
    return ReviewAttestationDecision(
        ...
        summary_content_hash=(
            ...
            if summary_content_hash is None
            else summary_content_hash
        ),
        advisory_only=True,
    )
...
def exact_review_evidence(
    *,
    ...
    review_summary = (
        exact_review_summary(review_request) if summary is None else summary
    )
    review_decision = (
        exact_review_decision(review_request, review_summary)
        if decision is None
        else decision
    )
    envelope = (
...
def exact_dependencies(
    *,
    ...
    return replace(
        initial,
        observed_dependency_chain_sha256=(
            initial.computed_dependency_chain_sha256
        ),
    )
...
def exact_ready_request(
    *,
    ...
    return MergeReadinessRequest(
        candidate=(
            exact_candidate(dependency_value) if candidate is None else candidate
        ),
        review=exact_review_evidence() if review is None else review,
        verification=(
            exact_verification() if verification is None else verification
        ),
    ...
    if case in {"review_blocked", "review_evidence_blocked"}:
        ...
        summary = exact_review_summary(
            ...
            evidence_blocked_count=(
                1 if case == "review_evidence_blocked" else 0
            ),
        )
...
def test_freshness_threshold_equality_passes_and_nonfinite_values_fail() -> None:
    request = exact_ready_request(
        verification=exact_verification(evidence_age_seconds=3600.0)
    )
