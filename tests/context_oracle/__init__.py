...
def solve_context_oracle(
    ...
    """Enumerate every feasible subset and recompute its objective independently."""

    ids = tuple(pool.evidence_by_id)
    mandatory = set(pool.mandatory_ids)
    for memory_id in pool.mandatory_ids:
        mandatory.update(pool.prerequisite_closure[memory_id])
    mandatory_closure = frozenset(mandatory)
...
def _is_feasible(
    ...
    if not mandatory_closure.issubset(selected):
        return False
    if any(
    ...
    for item in evidence:
        if item.memory_id in mandatory_closure:
            continue
...
def _objective_key(
    ...
    direct = policy.direct_credit_weight * sum(item.direct_credit for item in evidence)
    inherited = sum(
        _clamp(
